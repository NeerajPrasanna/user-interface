...
void _ui_bar_increment(lv_obj_t * target, int val, int anm)
{
    ...
    lv_bar_set_value(target, old + val, static_cast<lv_anim_enable_t>(anm));
}

void _ui_slider_increment(lv_obj_t * target, int val, int anm)
{
    int old = lv_slider_get_value(target);
    lv_slider_set_value(target, old + val, static_cast<lv_anim_enable_t>(anm));
    lv_event_send(target, LV_EVENT_VALUE_CHANGED, 0);
}
...

{
     ...
     lv_obj_t ** var = static_cast<lv_obj_t **>(lv_event_get_user_data(e));
    lv_obj_del(*var);
    (*var) = NULL;
}
